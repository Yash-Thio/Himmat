...
import { useSos, useUser } from "@/lib/auth-client";
...
export default function SOSButton({ className }: SOSButtonProps) {
  ...
  const startLocationTracking = useCallback(() => {
    ...
    const watchId = navigator.geolocation.watchPosition(
      (position) => {
        ...
        fetch(`${process.env.NEXT_PUBLIC_BACKEND_BASE_URL}/sos/start`, {
          method: "POST",
          headers: {
      ...
      (error) => {
        ...
        toast.error(
          "Failed to get location. Please check your location permissions.",
        );
  ...
  const stopLocationTracking = useCallback(() => {
    ...
    if (user) {
      fetch(`${process.env.NEXT_PUBLIC_BACKEND_BASE_URL}/sos/stop`, {
        method: "POST",
        headers: {
          "x-user-id": user.id.toString(),
        },
    ...
    if (isSosActive) {
      ...
      try {
        ...
        if (result.data?.sendSos) {
          ...
          toast.success(
            "SOS activated! Emergency contacts have been notified.",
            { id: "sos-loading" },
          );
        } else {
